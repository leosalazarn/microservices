...
A traditional CRUD approach (direct `save()` to MongoDB) was the simpler alternative, but fails on audit and
event-driven requirements without additional ceremony.
...
- Event versioning — renaming event classes breaks deserialization of stored events (mitigated by storing FQCN, see
  Phase 3)
